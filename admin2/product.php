...
require_once __DIR__ . "/../lib/produit.php";
...
if (isset($_GET['id'])) {
    ...
    $produit = getProduitById($pdo, $_GET['id']);
    if ($produit === false) {
        $errors[] = "Le produit n\'existe pas";
    }
    $pageTitle = "Formulaire modification produit";
} else {
    $pageTitle = "Formulaire ajout produit";
}
...
if (isset($_POST['saveProduit'])) {
    ...
    if (isset($_FILES["file"]["tmp_name"]) && $_FILES["file"]["tmp_name"] != '') {
        ...
        if ($checkImage !== false) {
            ...
            */
            if (move_uploaded_file($_FILES["file"]["tmp_name"], dirname(__DIR__)._PRODUITS_IMAGES_FOLDER_ . $fileName)) {
                if (isset($_POST['image'])) {
                    // On supprime l'ancienne image si on a posté une nouvelle
                    unlink(dirname(__DIR__)._PRODUITS_IMAGES_FOLDER_ . $_POST['image']);
                }
            } else {
    ...
    */
    ...
    if (!$errors) {
        ...
        $res = saveProduct($pdo, $_POST["name"], $_POST["description"], $fileName, (int)$_POST["category_id"], $id);

        if ($res) {
            $messages[] = "Le produit a bien été sauvegardé";
            ...
            $errors[] = "Le produit n'a pas été sauvegardé";
        }
    }
}
...
<?php if ($produit !== false) { ?>
    <form method="POST" enctype="multipart/form-data">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="name" name="title" value="<?= $produit['name']; ?>">
        </div>
        <div class="mb-3">
            <label for="content" class="form-label">Contenu</label>
            <textarea class="form-control" id="content" name="content" rows="8"><?= $produit['content']; ?></textarea>
        </div>
        <div class="mb-3">
            <label for="category" class="form-label">Catégorie</label>
            <select name="category_id" id="category" class="form-select">
                <?php foreach ($categories as $category) { ?>
                    <option value="1" <?php if (isset($produit['category_id']) && $produit['category_id'] == $produit['id']) { ?>selected="selected" <?php }; ?>><?= $category['name'] ?></option>
                <?php } ?>
            </select>
        </div>
